...
double bdFunctionalEval(
  ...
  const lf::assemble::DofHandler &dofh{fe_space->LocGlobMap()};
  auto mesh = dofh.Mesh();
  //std::shared_ptr<const lf::mesh::Mesh> mesh{fe_space->Mesh()}; //we can also initialise mesh in that way
  auto bd_flags{lf::mesh::utils::flagEntitiesOnBoundary(mesh, 1)};
  //auto edge_predicate = [&bd_flags](const lf::mesh::Entity *edge) -> bool{return bd_flags{*edge}};
  
  for(const lf::mesh::Entity *edge: mesh->Entities(1)){
      if(bd_flags(*edge)){
      auto endpoints = lf::geometry::Corners(*(edge->Geometry()));
      const double length_edge = (endpoints.col(1) - endpoints.col(0)).norm();
      auto dof_idx = dofh.GlobalDofIndices(*edge);
      const double p1 = coeff_vec(dof_idx[0]);
      const double p2 = coeff_vec(dof_idx[1]);
      bd_functional_val += 0.5 * length_edge * (p1 + p2);
      }
  }
